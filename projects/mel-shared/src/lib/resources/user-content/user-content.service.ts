...
export class UserContentService {
  ...
  constructor(
    ...
    // @TODO: Move listen features to an external service.
  ...
  getUserContentFromBackend(): Observable<UserContent> {
    return this.http.get<UserContent>(`${this.env.domains.mel}/api/resources/user-contents/me`, {
  ...
  update(): Observable<UserContent> {
    const obs = this.http.patch<UserContent>(`${this.env.domains.mel}/api/resources/user-contents`, this.content, {
      headers: new HttpHeaders({
        Authorization: 'auto'
      })
    }).pipe(shareReplay(1));
